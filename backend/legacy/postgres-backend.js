...
const authLimiter = rateLimit({
  ...
  message: {
    success: false,
    message: 'Çok fazla deneme yaptýnýz. Lütfen 15 dakika sonra tekrar deneyin.',
  },
...
const uploadLimiter = rateLimit({
  ...
  message: {
    success: false,
    message: 'Çok fazla dosya yükleme isteði. Lütfen bekleyin.',
  },
  standardHeaders: true,
  legacyHeaders: false,
});
...
const paymentLimiter = rateLimit({
  ...
  message: {
    success: false,
    message: 'Ödeme istek limiti aþýldý. Lütfen daha sonra tekrar deneyin.',
  },
  standardHeaders: true,
  legacyHeaders: false,
});
...
const verificationLimiter = rateLimit({
  ...
  message: {
    success: false,
    message: 'Çok fazla doðrulama kodu istendi. Lütfen bekleyin.',
  },
  standardHeaders: true,
  legacyHeaders: false,
});
...
async function seedData() {
  ...
  try {
    ...
    `,
      [
        userId,
        'Ýstanbul - Ankara Kargo',
        'Ofis malzemeleri taþýmacýlýðý',
        'Ýstanbul',
        'Ankara',
        100,
        2.5,
    ...
    `,
      ...
      try {
        await createNotification(
          ...
          'shipment_created',
          'Gönderi Oluþturuldu',
          `Gönderiniz baþarýyla oluþturuldu. Takip numaranýz: ${trackingNumber}`,
      ...
      res.status(201).json({
        success: true,
        message: 'Gönderi baþarýyla oluþturuldu',
        data: {
          shipment: shipment,
          id: shipment.id,
        },
...
app.post('/api/auth/register', authLimiter, async (req, res) => {
  try {
    ...
    if ((role === 'corporate' || role === 'nakliyeci') && !taxNumber) {
      return res.status(400).json({
        success: false,
        message: 'Vergi numarasý (VKN) zorunludur',
      });
    }
    ...
    if ((role === 'corporate' || role === 'nakliyeci') && taxNumber) {
      ...
      if (!vknRegex.test(taxNumber)) {
        return res.status(400).json({
          success: false,
          message: 'Vergi numarasý 10 haneli olmalýdýr',
        });
      }
    ...
    if (role === 'tasiyici' && !tckn) {
      return res.status(400).json({
        success: false,
        message: 'TC Kimlik No zorunludur (ehliyet doðrulamasý için)',
      });
    }
    ...
    if (tckn) {
      ...
      if (digits[9] !== q1 || digits[10] !== q2) {
        return res.status(400).json({
          success: false,
          message: 'Geçersiz TC Kimlik No (checksum hatasý)',
        });
      }
    }
    ...
    const emailSent = await sendEmail(
      email,
      'YolNext - Email Doðrulama',
      `YolNext'e hoþ geldiniz!\n\nEmail adresinizi doðrulamak için aþaðýdaki linke týklayýn:\n\n${verificationUrl}\n\nBu link 24 saat geçerlidir.\n\nEðer bu hesabý siz oluþturmadýysanýz, bu emaili görmezden gelebilirsiniz.`
    );

    if (!emailSent) {
      console.warn('?? Email gönderilemedi, ancak token oluþturuldu');
    }
    ...
    await createNotification(
      ...
      'welcome',
      'Hoþ geldiniz!',
      `${user.fullName || user.email}, YolNext ailesine hoþ geldiniz! Email adresinizi doðrulamayý unutmayýn.`,
      '/dashboard',
      'normal',
      { type: 'system' }
    );
    ...
    res.status(201).json({
      success: true,
      message: 'Kayýt baþarýlý. Email doðrulama linki gönderildi.',
      data: {
        user: {
          id: user.id,
          email: user.email,
  ...
  try {
    ...
    if (req.user.isDemo) {
      result = await pool.query(
        `
        SELECT s.*, 
               COALESCE(u.fullName, 'Demo Kullanýcý') as ownerName,
...
app.post(
  ...
  async (req, res) => {
    ...
    `,
      ...
      res.status(201).json({
        success: true,
        message: 'Teklif baþarýyla verildi',
        data: offer,
      });
    } catch (error) {
  ...
  async (req, res) => {
    ...
    `,
      ...
      if (offer.status !== 'pending') {
        return res.status(400).json({
          success: false,
          message: 'Teklif artýk geçerli deðil',
        });
      }
      ...
      if (offer.shipmentStatus !== 'open') {
        return res.status(400).json({
          success: false,
          message: 'Gönderi artýk açýk deðil',
        });
      }
      ...
      `,
        ...
        await createNotification(
          ...
          'offer_accepted',
          'Teklif Kabul Edildi',
          `Teklifiniz kabul edildi. Gönderi hazýrlanýyor.`,
        ...
        await createNotification(
          ...
          `Gönderiniz için verdiðiniz teklif kabul edildi. Gönderi detaylarýný inceleyin.`,
          `/shipments/${offer.shipmentId}`,
          'high',
          { shipmentId: offer.shipmentId, offerId }
        );
        ...
        res.json({
          success: true,
          message: 'Teklif baþarýyla kabul edildi',
...
app.post('/api/complaints', authenticateToken, async (req, res) => {
  try {
    ...
    await createNotification(
      ...
      'complaint',
      'Yeni þikayet',
      `Size karþý bir þikayet oluþturuldu. Gönderi #${shipmentId}`,
    ...
    res.json({
      success: true,
      message: 'Þikayet baþarýyla oluþturuldu',
      data: result.rows[0],
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      error: NODE_ENV === 'production' ? 'Þikayet oluþturulamadý' : error.message,
    });
  }
});
...
app.get('/api/complaints', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: NODE_ENV === 'production' ? 'Þikayetler alýnamadý' : error.message,
    });
  }
});
...
app.post('/api/ratings', authenticateToken, async (req, res) => {
  try {
    ...
    await createNotification(
      ...
      'rating_received',
      'Yeni Deðerlendirme',
      `Gönderiniz için ${rating} yýldýz deðerlendirme aldýnýz.`,
      `/ratings`,
      'normal',
    ...
    res.json({
      success: true,
      message: 'Deðerlendirme baþarýyla kaydedildi',
      data: result.rows[0],
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      error: 'Deðerlendirme kaydedilemedi',
      details: error.message,
    });
  }
});
...
app.post('/api/verify/email/send', verificationLimiter, async (req, res) => {
  try {
    ...
    if (userResult.rows[0].isEmailVerified) {
      return res.status(400).json({ success: false, message: 'Email zaten doðrulanmýþ' });
    }
    ...
    res.json({
      success: true,
      message: 'Doðrulama emaili gönderildi',
      expiresIn: '24 hours'
    });
  } catch (error) {
...
app.post('/api/verify/email', async (req, res) => {
  try {
    ...
    if (tokenResult.rows.length === 0) {
      return res.status(400).json({ success: false, message: 'Geçersiz veya süresi dolmuþ token' });
    }
    ...
    try {
      ...
      await createNotification(
        ...
        'email_verified',
        'Email Doðrulandý',
        'Email adresiniz baþarýyla doðrulandý.',
        '/settings',
        'normal',
        {}
      );
      ...
      res.json({
        success: true,
        message: 'Email baþarýyla doðrulandý'
      });
    } catch (error) {
      await pool.query('ROLLBACK');
      throw error;
...
app.post('/api/verify/phone/send', verificationLimiter, async (req, res) => {
  try {
    ...
    if (!phone) {
      return res.status(400).json({ success: false, message: 'Telefon numarasý gerekli' });
    }

    // Format phone number (Turkish format: 5XXXXXXXXX)
    const cleanPhone = phone.replace(/\D/g, '');
    if (cleanPhone.length !== 10) {
      return res.status(400).json({ success: false, message: 'Geçersiz telefon numarasý formatý' });
    }
    ...
    res.json({
      success: true,
      message: 'Doðrulama kodu gönderildi',
...
app.post('/api/verify/phone', async (req, res) => {
  try {
    ...
    if (codeResult.rows.length === 0) {
      ...
      return res.status(400).json({ success: false, message: 'Geçersiz veya süresi dolmuþ kod' });
    }
    ...
    if (verification.attempts >= 5) {
      return res.status(429).json({ success: false, message: 'Çok fazla deneme yapýldý' });
    }
    ...
    try {
      ...
      res.json({
        success: true,
        message: 'Telefon numarasý baþarýyla doðrulandý'
      });
    } catch (error) {
      await pool.query('ROLLBACK');
      throw error;
...
app.post('/api/upload', authenticateToken, uploadLimiter, upload.single('file'), async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: 'Dosya baþarýyla yüklendi',
      data: {
        url: fullUrl,
  ...
  async (req, res) => {
    try {
      ...
      if (s.rows.length === 0)
        return res
          .status(404)
          .json({ success: false, message: 'Gönderi bulunamadý' });
      ...
      //   if (!isOwner && !isCarrier) {
      //     return res.status(403).json({ success: false, message: 'Yetkisiz iþlem' });
      //   }
      // }
      ...
      if (shipmentCarrierId && !isNaN(shipmentCarrierId)) {
        await createNotification(
          ...
          'shipment_delivered',
          'Teslimat Tamamlandý',
          'Teslim ettiðiniz gönderi tamamlandý.',
          `/shipments/${shipmentId}`,
          'high',
          { shipmentId }
        );
      ...
      res.json({ success: true, message: 'Teslim edildi olarak iþaretlendi' });
    } catch (e) {
      res.status(500).json({ success: false, error: e.message });
    }
  }
  ...
  async (req, res) => {
    try {
      ...
      if (!carrierId)
        return res
          .status(400)
          .json({ success: false, message: 'Atanacak taþýyýcý bulunamadý' });
  ...
  async (req, res) => {
    try {
      ...
      await createNotification(
        ...
        'broadcast_opened',
        'Ýlan Açýldý',
        'Gönderiniz taþýyýcýlar için açýldý',
...
app.post('/api/users/me/delete', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: 'Hesap silme talebiniz iþlendi (soft delete + anonimleþtirme)',
    });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
...
app.get('/api/payments/shipment/:shipmentId', authenticateToken, async (req, res) => {
  try {
    ...
    if (shipmentResult.rows.length === 0) {
      return res.status(404).json({ success: false, message: 'Gönderi bulunamadý' });
    }

    const shipment = shipmentResult.rows[0];
    if (parseInt(shipment.userid) !== parseInt(userId) && 
        (!shipment.carrierid || parseInt(shipment.carrierid) !== parseInt(userId))) {
      return res.status(403).json({ success: false, message: 'Yetkisiz eriþim' });
    }
...
app.post('/api/payments/shipment/:shipmentId/pay', authenticateToken, paymentLimiter, paymentSpeedLimiter, async (req, res) => {
  try {
    ...
    if (data.payment_id && data.payment_status === 'paid') {
      return res.status(400).json({ success: false, message: 'Ödeme zaten yapýlmýþ' });
    }
    ...
    try {
      ...
      if (paymentMethod === 'wallet') {
        ...
        if (walletResult.rows.length === 0) {
          await pool.query('ROLLBACK');
          return res.status(400).json({ success: false, message: 'Cüzdan bulunamadý' });
        }
        ...
        if (paymentResponse.status !== 'success') {
          ...
          return res.status(400).json({
            success: false,
            message: paymentResponse.errorMessage || 'Ödeme iþlemi baþarýsýz'
          });
        }
      } else {
        await pool.query('ROLLBACK');
      ...
      await createNotification(
        ...
        'payment_completed',
        'Ödeme Tamamlandý',
        `Gönderi ödemesi baþarýyla tamamlandý. Tutar: ${amount.toFixed(2)} TL`,
        `/shipments/${shipmentId}`,
        'normal',
      ...
      if (data.carrierid) {
        await createNotification(
          ...
          'payment_received',
          'Ödeme Alýndý',
          `Gönderi ödemesi alýndý. Gönderiyi tamamladýðýnýzda ödeme serbest býrakýlacak.`,
          `/shipments/${shipmentId}`,
          'normal',
          { shipmentId, amount }
        );
...
app.post('/api/payments/shipment/:shipmentId/release', authenticateToken, paymentLimiter, paymentSpeedLimiter, async (req, res) => {
  try {
    ...
    if (data.payment_status !== 'paid') {
      return res.status(400).json({ success: false, message: 'Ödeme henüz yapýlmamýþ' });
    }
    ...
    if (data.status !== 'delivered') {
      return res.status(400).json({ 
        success: false, 
        message: 'Ödeme serbest býrakýlmadan önce gönderi teslim edilmiþ olmalý' 
      });
    }

    if (!data.carrierid) {
      return res.status(400).json({ success: false, message: 'Nakliyeci bulunamadý' });
    }
    ...
    try {
      ...
      await pool.query(
        ...
        [wallet.id, data.carrierid, carrierAmount, `Gönderi #${shipmentId} ödeme serbest býrakýldý`]
      );
      ...
      await createNotification(
        parseInt(data.carrierid),
        'payment_released',
        'Ödeme Serbest Býrakýldý',
        `Gönderi ödemesi cüzdanýnýza yatýrýldý. Tutar: ${carrierAmount.toFixed(2)} TL`,
        `/shipments/${shipmentId}`,
        'high',
      ...
      res.json({
        success: true,
        message: 'Ödeme nakliyeciye serbest býrakýldý',
        data: {
...
app.post('/api/support/tickets', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: 'Destek talebi oluþturuldu',
      data: result.rows[0],
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      error: NODE_ENV === 'production' ? 'Destek talebi oluþturulamadý' : error.message,
    });
  }
});
...
app.get('/api/support/tickets', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: NODE_ENV === 'production' ? 'Destek talepleri alýnamadý' : error.message,
    });
  }
});
