...
async function loginAsDemoUser(page, userType) {
  try {
    ...
    try {
      await page.waitForSelector(`button[data-testid="demo-${userType}"]`, { timeout: 10000 });
    } catch (e) {
      // If specific button not found, try to find any demo button
      await page.waitForSelector('button', { timeout: 10000 });
    }
    
    // Try clicking the specific demo button
    const buttonSelector = `button[data-testid="demo-${userType}"]`;
    try {
      ...
      try {
        ...
        console.error(`Could not find demo button for ${userType}`);
      }
    }
