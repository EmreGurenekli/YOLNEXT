...
const testAgreementSystem = async () => {
  ...
  try {
    // Create Agreement
    const agreementData = {
      offer_id: 1 // Assuming offer exists
    };
    
    const createResponse = await makeRequest('POST', '/agreements', agreementData, {
      Authorization: `Bearer ${individualUser.token}`
    });
    
    if (createResponse.status !== 201) {
      throw new Error(`Agreement creation failed: ${createResponse.status}`);
    }
    
    log('✅ Agreement Creation Passed', 'success');
    testResults.passed++;
...
const testTrackingSystem = async () => {
  ...
  try {
    ...
    const updateResponse = await makeRequest('PUT', '/tracking/1/status', statusData, {
      Authorization: `Bearer ${nakliyeciUser.token}`
    });
    ...
    log('✅ Status Update Passed', 'success');
    testResults.passed++;
    
    // Get Tracking History
    const historyResponse = await makeRequest('GET', '/tracking/1/history', null, {
      Authorization: `Bearer ${nakliyeciUser.token}`
    });
    
    if (historyResponse.status !== 200) {
      throw new Error(`Tracking history retrieval failed: ${historyResponse.status}`);
    }
    
    log('✅ Tracking History Passed', 'success');
    testResults.passed++;
