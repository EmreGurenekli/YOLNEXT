...
router.post('/create', authenticateToken, [
  ...
  body('payment_method').isIn(['credit_card', 'bank_transfer', 'wallet']).withMessage('Geçerli ödeme yöntemi gerekli')
], async (req, res) => {
  try {
    ...
    if (payment_method === 'wallet') {
      ...
      await db.run('UPDATE wallets SET balance = balance - ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?', 
        [amount, req.user.id]);
      ...
      `, [wallet.id, req.user.id, amount, wallet.balance, wallet.balance - amount, order_id, 
      ...
      // Banka havalesi
      const bankAccount = {
        bank_name: 'YolNet Bankası',
        account_name: 'YolNet Kargo A.Ş.',
      ...
      res.json({
        ...
        data: {
          ...
          instructions: 'Lütfen açıklama kısmına ödeme referansınızı yazın: ' + payment_reference
        }
      });
    }
...
router.get('/status/:payment_reference', authenticateToken, async (req, res) => {
  try {
...
router.get('/wallet/balance', authenticateToken, async (req, res) => {
  try {
    ...
    if (!wallet) {
      // Cüzdan oluştur
      const result = await db.run('INSERT INTO wallets (user_id) VALUES (?)', [req.user.id]);
      const newWallet = await db.get('SELECT * FROM wallets WHERE id = ?', [result.lastID]);
...
router.post('/wallet/deposit', authenticateToken, [
  body('amount').isFloat({ min: 10 }).withMessage('Minimum 10 TL yatırabilirsiniz'),
  body('payment_method').isIn(['credit_card', 'bank_transfer']).withMessage('Geçerli ödeme yöntemi gerekli')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    let wallet = await db.get('SELECT * FROM wallets WHERE user_id = ?', [req.user.id]);
    if (!wallet) {
      const result = await db.run('INSERT INTO wallets (user_id) VALUES (?)', [req.user.id]);
      wallet = await db.get('SELECT * FROM wallets WHERE id = ?', [result.lastID]);
    }
    ...
    `, [wallet.id, req.user.id, amount, wallet.balance, wallet.balance + amount, 
...
async function makeIyzicoPayment(request) {
  // Bu fonksiyon gerçek İyzico API entegrasyonu içerir
