...
export const useShipmentStore = create<ShipmentStore>((set: any, get: any) => ({
  ...
  setShipments: (shipments: Shipment[]) => {
    logger.info('Shipments updated', { count: shipments.length });
    set({ shipments });
  },

  addShipment: (shipment: Shipment) => {
    const { shipments } = get();
    logger.info('Shipment added', { shipmentId: shipment.id });
  ...
  updateShipment: (id: string, updates: Partial<Shipment>) => {
    ...
    logger.info('Shipment updated', { shipmentId: id, updates });
  ...
  removeShipment: (id: string) => {
    ...
    logger.info('Shipment removed', { shipmentId: id });
    set({ shipments: filteredShipments });
  },

  setOffers: (offers: Offer[]) => {
    logger.info('Offers updated', { count: offers.length });
    set({ offers });
  },

  addOffer: (offer: Offer) => {
    const { offers } = get();
    logger.info('Offer added', {
      offerId: offer.id,
  ...
  updateOffer: (id: string, updates: Partial<Offer>) => {
    ...
    logger.info('Offer updated', { offerId: id, updates });
    set({ offers: updatedOffers });
  },

  setOpenShipments: (shipments: Shipment[]) => {
    logger.info('Open shipments updated', { count: shipments.length });
  ...
  setError: (error: string | null) => {
    if (error) {
      logger.error('Shipment store error', { error });
    }
    set({ error });
  },
