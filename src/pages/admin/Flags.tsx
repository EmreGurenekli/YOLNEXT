  ...
  const load = async () => {
    ...
    try {
      ...
      if (e?.name !== 'AbortError' && seq === loadSeqRef.current) {
        setError(e?.message || 'Flags yüklenemedi');
      }
    } finally {
      ...
      {createOpen && (
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
          ...
          <div className='relative w-full max-w-lg bg-white rounded-2xl shadow-xl border border-gray-200 p-6'>
            ...
            <div className='mt-4'>
              <div className='text-xs font-semibold text-slate-500 uppercase tracking-wider'>Target ID</div>
              <input
                className='input mt-2'
                value={targetId}
                onChange={e => setTargetId(e.target.value)}
                placeholder='Örn: userId'
              />
            </div>
