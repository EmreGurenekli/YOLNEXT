...
const IndividualMessages: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (message: any) => {
      setConversations(prev => {
        ...
        if (conversationIndex !== -1) {
          ...
          updated[conversationIndex].lastMessage = message.message;
          updated[conversationIndex].lastMessageTime = new Date(
            message.createdAt
    ...
    //   socketService.getSocket()?.off('message:read', handleMessageRead);
    // };
  }, []);
    ...
    if (newMessage.trim() && selectedConversation) {
      try {
        const response = await messageAPI.send({
          receiverId: selectedConversation.carrierId,
          message: newMessage,
          type: 'text',
