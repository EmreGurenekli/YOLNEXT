...
export default function CorporateMessages() {
  ...
  const filteredConversations = conversations.filter(conv => {
    const matchesSearch =
      conv.carrier.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <>
      <Helmet>
        <title>Mesajlar - YolNext Kurumsal</title>
        <meta
          name='description'
          content='Nakliyecilerle iletişim kurun ve mesajlaşın'
        />
      </Helmet>
        ...
        <div className='max-w-7xl mx-auto px-4 py-6'>
          ...
          <div className='text-center mb-12'>
            ...
            <p className='text-xl text-slate-600 max-w-3xl mx-auto leading-relaxed'>
              Nakliyecilerinizle profesyonel iletişim kurun ve gönderilerinizi
              takip edin
            </p>
          </div>
                ...
                <div className='flex items-center gap-4'>
                  <div>
                    ...
                    <p className='text-slate-600'>
                      Nakliyecilerinizle gerçek zamanlı iletişim
                    </p>
                  </div>
                  ...
                  {filteredConversations.length === 0 ? (
                    <div className='flex flex-col items-center justify-center h-full p-8 text-center'>
                      ...
                      <p className='text-slate-500 text-sm'>
                        Nakliyecilerinizle mesajlaşmaya başlayın
                      </p>
                    </div>
                  ) : (
                          ...
                          <div className='relative'>
                            ...
                            <div
                              className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${
                                conversation.carrier.status === 'online'
                                  ? 'bg-emerald-500'
                                  : 'bg-slate-400'
                              }`}
                            ></div>
                          ...
                          <div className='flex-1 min-w-0'>
                            <div className='flex items-center justify-between mb-1'>
                              <h3 className='font-semibold text-slate-900 truncate'>
                                {conversation.carrier.name}
                              </h3>
                              <div className='flex items-center gap-1'>
                                <Star className='w-3 h-3 text-amber-500' />
                                <span className='text-xs text-slate-500'>
                                  {conversation.carrier.rating}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className='flex items-center justify-between'>
                              <span
                                className={`text-xs px-2 py-1 rounded-full ${
                                  conversation.shipment.status === 'Yolda'
                      ...
                      <div className='flex items-center justify-between'>
                        <div className='flex items-center gap-4'>
                          <div className='relative'>
                            ...
                            <div
                              className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${
                                selectedConversation.carrier.status === 'online'
                                  ? 'bg-emerald-500'
                                  : 'bg-slate-400'
                              }`}
                            ></div>
                          ...
                          <div>
                            <h3 className='text-lg font-bold text-slate-900'>
                              {selectedConversation.carrier.name}
                            </h3>
                            <div className='flex items-center gap-2'>
                              <div className='flex items-center gap-1'>
                                <Star className='w-4 h-4 text-amber-500' />
                                <span className='text-sm text-slate-600'>
                                  {selectedConversation.carrier.rating}
                                </span>
                              </div>
                              <span className='text-sm text-slate-500'>•</span>
                              <span className='text-sm text-slate-500'>
                                {selectedConversation.carrier.lastSeen}
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className='flex items-center justify-between'>
                          <div>
                            <h4 className='font-semibold text-slate-900'>
                              {selectedConversation.shipment.title}
                            </h4>
                            <div className='flex items-center gap-4 mt-1'>
                              <div className='flex items-center gap-1 text-sm text-slate-600'>
                                <MapPin className='w-4 h-4' />
                                {selectedConversation.shipment.from} →{' '}
                                {selectedConversation.shipment.to}
                              </div>
                              <div className='flex items-center gap-1 text-sm text-slate-600'>
                          ...
                          <span
                            className={`px-3 py-1 rounded-full text-sm font-medium ${
                              selectedConversation.shipment.status === 'Yolda'
                                ? 'bg-blue-100 text-blue-700'
