...
export default function CorporateCarriers() {
  ...
  const [selectedCarrierForAgreement, setSelectedCarrierForAgreement] = useState<number | null>(null);
  const [selectedCarrierForCommunication, setSelectedCarrierForCommunication] = useState<number | null>(null);
  ...
  const handleBulkAction = (action: string) => {
    ...
    alert(`${action} işlemi ${selectedCarriers.length} nakliyeci için uygulanacak`);
  };
  ...
  const carriers = [
    {
      ...
      specialties: ["Soğuk Zincir", "Kırılabilir", "Acil"],
      contact: {
        phone: "+90 555 123 45 67",
        email: "info@kargoexpress.com",
        website: "www.kargoexpress.com"
      },
      status: "active",
    ...
    },
    {
      ...
      totalSpent: 13225,
      location: "İzmir",
      coverage: ["İzmir", "Bursa", "Antalya", "Denizli"],
      specialties: ["Tekstil", "Elektronik", "Güvenli"],
      contact: {
    ...
    },
    {
      ...
      totalSpent: 13945,
      location: "Ankara",
      coverage: ["Ankara", "İstanbul", "Konya", "Eskişehir"],
      specialties: ["Tehlikeli Madde", "Değerli Eşya", "Güvenli"],
      contact: {
        phone: "+90 312 789 01 23",
        email: "info@guvenlitasimacilik.com",
        website: "www.guvenlitasimacilik.com"
      },
      status: "active",
      contractType: "preferred",
      ...
      employees: 65,
      established: "2012",
      certifications: ["ISO 9001", "ISO 14001", "OHSAS 18001", "ADR"],
      performance: {
        quality: 4.8,
        reliability: 4.9,
        communication: 4.6,
    ...
    },
    {
      ...
      totalSpent: 7821,
      location: "Bursa",
      coverage: ["Bursa", "İstanbul", "İzmir", "Balıkesir"],
      specialties: ["Gıda", "Hızlı", "Ekonomik"],
      contact: {
        phone: "+90 224 345 67 89",
        email: "info@megakargo.com",
        website: "www.megakargo.com"
      },
      status: "inactive",
    ...
    },
    {
      ...
      totalSpent: 7056,
      location: "Antalya",
      coverage: ["Antalya", "Mersin", "Adana", "Hatay"],
      specialties: ["Turizm", "Lüks", "Özel"],
      contact: {
        phone: "+90 242 567 89 01",
        email: "info@ultralojistik.com",
        website: "www.ultralojistik.com"
      },
      status: "pending",
      contractType: "general",
  ...
  const filteredCarriers = carriers.filter(carrier => {
    const matchesSearch = carrier.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         ...
                         carrier.specialties.some(s => s.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
      <Helmet>
        <title>Nakliyeci Yönetimi - YolNet Kargo</title>
      ...
      <div className="max-w-5xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
        ...
        <div className="text-center mb-8 sm:mb-12">
          ...
          <p className="text-sm sm:text-base md:text-lg text-slate-600 px-4">Anlaşmalı nakliyecilerinizi yönetin ve performanslarını takip edin</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 mb-6 sm:mb-8">
          <button 
            ...
            className="px-5 py-2.5 border border-slate-300 text-slate-700 rounded-lg hover:bg-slate-50 transition-colors flex items-center gap-2 text-sm shadow-sm"
          >
          ...
          <button 
            ...
            className="px-5 py-2.5 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors flex items-center gap-2 text-sm shadow-md"
          >
        ...
        {isLoading ? (
          <LoadingState text="Nakliyeciler yükleniyor..." />
          ...
          <>
                ...
                <div className="bg-white rounded-2xl p-8 shadow-xl border border-slate-200">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {paginatedCarriers.map((carrier) => (
                      <div key={carrier.id} className="bg-white rounded-xl p-6 border border-slate-200 hover:shadow-lg hover:border-slate-300 transition-all duration-300 group">
                        {/* Header Section */}
                        <div className="flex items-start justify-between mb-6">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <h3 className="text-lg font-bold text-slate-900 mb-1">{carrier.name}</h3>
                              <div className="flex items-center gap-2">
                                <MapPin className="w-4 h-4 text-slate-400" />
                                <span className="text-sm text-slate-600 font-medium">{carrier.location}</span>
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center justify-between mb-6">
                          <div className="flex items-center gap-2">
                            <div className="flex items-center gap-1">
                              ...
                              <span className="font-bold text-slate-900 text-lg">{carrier.rating}</span>
                            </div>
                            <div className="w-px h-4 bg-slate-300"></div>
                            <span className="text-sm text-slate-500 font-medium">{carrier.totalShipments} gönderi</span>
                          </div>
                          <span className={`px-3 py-1.5 text-xs font-semibold rounded-lg border ${getStatusColor(carrier.status)}`}>
                            {carrier.status === 'active' ? 'Aktif' : carrier.status === 'inactive' ? 'Pasif' : 'Beklemede'}
                          </span>
                        </div>
                        ...
                        <div className="bg-slate-50 rounded-lg p-4 mb-6">
                          <div className="grid grid-cols-3 gap-4">
                            <div className="text-center">
                              <div className="text-lg font-bold text-slate-900">%{carrier.onTimeDelivery}</div>
                              <div className="text-xs text-slate-500 font-medium">Zamanında Teslimat</div>
                            </div>
                            <div className="text-center">
                              <div className="text-lg font-bold text-slate-900">₺{carrier.averageCost}</div>
                              <div className="text-xs text-slate-500 font-medium">Ortalama Maliyet</div>
                            </div>
                            <div className="text-center">
                              <div className="text-lg font-bold text-slate-900">₺{(carrier.totalSpent / 1000).toFixed(0)}K</div>
                              <div className="text-xs text-slate-500 font-medium">Toplam Harcama</div>
                            </div>
                          </div>
                        </div>
                        ...
                        {/* Specialties */}
                        <div className="mb-6">
                          <div className="text-sm text-slate-600 font-semibold mb-3">Uzmanlık Alanları</div>
                          <div className="flex flex-wrap gap-2">
                            {carrier.specialties.map((specialty, index) => (
                              <span key={index} className="px-3 py-1 bg-blue-50 text-blue-700 text-xs font-semibold rounded-lg border border-blue-200">
                                {specialty}
                              </span>
                            ))}
                          </div>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-slate-200">
                          <span className={`px-3 py-1.5 text-xs font-semibold rounded-lg ${getContractTypeColor(carrier.contractType)}`}>
                            {carrier.contractType === 'preferred' ? 'Öncelikli Partner' : 
                             carrier.contractType === 'contracted' ? 'Anlaşmalı Partner' : 'Genel Partner'}
                          </span>
                          <div className="flex items-center gap-1">
                            <button 
                            ...
                            <button 
                              onClick={() => handleContactCarrier(carrier.id, 'phone')}
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-slate-200">
                          ...
                          <th className="text-left py-3 px-4 font-semibold text-slate-700">Puan</th>
                          <th className="text-left py-3 px-4 font-semibold text-slate-700">Gönderi</th>
                          <th className="text-left py-3 px-4 font-semibold text-slate-700">Zamanında Teslimat</th>
                          <th className="text-left py-3 px-4 font-semibold text-slate-700">Maliyet</th>
                      ...
                      <tbody>
                        {paginatedCarriers.map((carrier) => (
                          <tr key={carrier.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                            ...
                            <td className="py-4 px-4">
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  ...
                                  <div className="text-sm text-slate-500">{carrier.location}</div>
                                </div>
                              </div>
                            </td>
                            <td className="py-4 px-4">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="font-medium text-slate-900">{carrier.rating}</span>
                              </div>
                            </td>
                            <td className="py-4 px-4">
                              <div className="text-sm text-slate-900">{carrier.totalShipments}</div>
                              <div className="text-xs text-slate-500">₺{carrier.totalSpent.toLocaleString()}</div>
                            </td>
                            <td className="py-4 px-4">
                              <div className="text-sm text-slate-900">%{carrier.onTimeDelivery}</div>
                            </td>
                            <td className="py-4 px-4">
                              <div className="text-sm text-slate-900">₺{carrier.averageCost}</div>
                            </td>
                            <td className="py-4 px-4">
                              <span className={`px-2 py-1 text-xs font-medium rounded-full border ${getStatusColor(carrier.status)}`}>
                                {carrier.status === 'active' ? 'Aktif' : carrier.status === 'inactive' ? 'Pasif' : 'Beklemede'}
                              </span>
                            </td>
                            <td className="py-4 px-4">
            ...
            <div className="lg:hidden space-y-4">
              {filteredCarriers.map((carrier) => (
                <div key={carrier.id} className="bg-white rounded-xl p-4 shadow-lg border border-slate-200">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-slate-900 text-lg">{carrier.name}</h3>
                        <div className="flex items-center gap-2">
                          <div className="flex items-center gap-1">
                          ...
                          <span className="text-sm text-slate-500">{carrier.location}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="text-center p-3 bg-slate-50 rounded-lg">
                      <div className="text-lg font-bold text-slate-900">{carrier.totalShipments}</div>
                      <div className="text-xs text-slate-500">Toplam Gönderi</div>
                    </div>
                    <div className="text-center p-3 bg-slate-50 rounded-lg">
                      <div className="text-lg font-bold text-slate-900">%{carrier.onTimeDelivery}</div>
                      <div className="text-xs text-slate-500">Zamanında Teslimat</div>
                    </div>
                  </div>
                  
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <div className="text-sm font-medium text-slate-900">Ortalama Maliyet</div>
                      <div className="text-lg font-bold text-green-600">₺{carrier.averageCost}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-medium text-slate-900">Toplam Harcama</div>
                      <div className="text-lg font-bold text-blue-600">₺{carrier.totalSpent.toLocaleString()}</div>
                    </div>
                  </div>
