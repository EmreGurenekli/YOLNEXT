...
const getApiBaseUrl = () => {
  // In dev (Vite), always use relative '/api' so the dev server proxy forwards to backend.
  // This avoids CORS/network issues when hitting an absolute backend origin from the browser.
  if (
  ...
  // In preview/prod builds (including Playwright's vite preview), there is no dev proxy.
  // If VITE_API_URL is not provided, default to local backend.
  if (
  typeof import.meta !== 'undefined' &&
  import.meta &&
  import.meta.env &&
  (import.meta.env.MODE === 'test' || import.meta.env.MODE === 'production') &&
...
const responseInterceptor = response => {
  ...
  if (response.status === 401) {
    ...
    if (!token || token === 'null' || token === 'undefined' || !token.startsWith('demo-token-')) {
...
export const api = {
  ...
  carriers: {
    getCorporate: () => apiCall('/carriers/corporate'),
    linkCorporate: data =>
      apiCall('/carriers/corporate/link', {
        method: 'POST',
        body: JSON.stringify(data),
      }),
  },
  ...
  messages: {
    ...
    deleteConversation: otherUserId =>
      apiCall(`/messages/conversation/${otherUserId}`, { method: 'DELETE' }),
  },
...
export const carriersAPI = api.carriers;
