class MockApiClient {
    ...
    async mockRequest(endpoint, options = {}) {
        ...
        switch (endpoint) {
            case '/api/health':
                return { status: 'OK', timestamp: new Date().toISOString(), message: 'YolNet Backend is running!' };
            case '/api/auth/login':
                ...
                if (demoUsers[email] && demoUsers[email].password === password) {
                    ...
                    this.setToken(token);
                    return { message: 'Login successful', token, user: demoUsers[email].user };
                }
                else {
                    throw new Error('Invalid credentials');
                }
            case '/api/auth/register':
                const { name, email: regEmail, password: regPassword, panel_type } = JSON.parse(options.body);
                const token = 'demo-token-' + Date.now();
                this.setToken(token);
                return { message: 'User created successfully', token, user: { id: Date.now(), name, email: regEmail, panel_type } };
            case '/api/auth/me':
                if (!this.token)
                    throw new Error('No token');
                return { user: { id: 1, name: 'Demo User', email: 'demo@example.com', panel_type: 'individual' } };
            case '/api/shipments':
                const mockShipments = [
                    {
                        id: 1,
            ...
            case '/api/messages':
                ...
                return mockMessages;
            case '/api/wallet':
                return { id: 1, user_id: 1, balance: 1250.50, currency: 'TRY', is_active: true };
    ...
    async login(credentials) {
        return this.mockRequest('/api/auth/login', {
            method: 'POST',
            body: JSON.stringify(credentials),
        });
    }
    async register(data) {
        return this.mockRequest('/api/auth/register', {
            method: 'POST',
            body: JSON.stringify(data),
        });
    }
    async verifyToken() {
        return this.mockRequest('/api/auth/me');
    }
    async logout() {
        this.setToken(null);
