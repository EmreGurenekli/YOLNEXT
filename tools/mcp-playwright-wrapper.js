...
function ensureArtifactsDir() {
  ...
  const base = join(os.tmpdir(), 'yolnext-mcp-playwright-');
  artifactsDir = fs.mkdtempSync(base);
  return artifactsDir;
}

function registerArtifact(path) {
  if (!path) return;
...
export async function initBrowser(options = {}) {
  ...
  try {
    // Use an isolated persistent context to avoid "browser is already in use" profile locks.
    // Each run gets a unique userDataDir under OS temp.
    if (!userDataDir) {
      const base = join(os.tmpdir(), 'yolnext-playwright-profile-');
      userDataDir = fs.mkdtempSync(base);
    }
...
export async function navigate(url, options = {}) {
  try {
    ...
    if (!page) {
      const existingPages = context ? context.pages() : [];
      page = existingPages && existingPages.length > 0 ? existingPages[0] : await (context ? context.newPage() : browser.newPage());
...
export async function snapshot(filename = 'snapshot.png') {
  try {
    ...
    const safeName = String(filename || 'snapshot.png').replace(/[\\/]/g, '_');
    const outPath = join(dir, safeName);
    registerArtifact(outPath);
    ...
    return { screenshot, title, url, path: outPath };
  } catch (error) {
...
export async function waitForNavigation(options = {}) {
  try {
    if (!page) throw new Error('No page loaded');
    const timeout = options.timeout || 30000;
    const state = options.state || 'domcontentloaded';
...
export async function close() {
  try {
    ...
    const toDelete = [...createdArtifacts];
