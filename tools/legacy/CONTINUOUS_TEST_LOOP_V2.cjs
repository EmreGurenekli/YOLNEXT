// CONTINUOUS TEST LOOP V2 - Enhanced with better error handling
...
async function testPage(page, url, testName) {
  try {
    ...
    if (!pageInfo.rootContent) {
      logResult('failed', `${testName}: Empty root`, url);
      return false;
    }
    
    if (consoleErrors.length > 0 && iteration > 1) {
      logResult('warnings', `${testName}: ${consoleErrors.length} console errors`, 
...
async function testDemoLogin(page) {
  try {
    ...
    if (await button.isVisible({ timeout: 3000 }).catch(() => false)) {
      await button.click();
      await page.waitForTimeout(6000); // Longer wait for redirect
...
async function runTestSuite(page) {
  ...
  // Test authenticated pages if demo login worked
  ...
  // Count errors from this iteration only
...
async function main() {
  console.log('\n🚀 CONTINUOUS TEST LOOP V2\n');
  console.log('Testing, fixing, and retesting until perfect...\n');
